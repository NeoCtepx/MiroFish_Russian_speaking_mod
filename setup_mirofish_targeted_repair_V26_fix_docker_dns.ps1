...
function Repair-DockerDesktopNetwork {
    Write-Host ""
    Write-Host "=== Docker Desktop network/DNS repair ==="
    Warn "Docker cannot resolve registry-1.docker.io. Build will fail before project code is used."

    Info "Flushing Windows DNS cache..."
    Invoke-External -Exe "ipconfig" -ArgList @("/flushdns") -AllowFail | Out-Null

    Info "Resetting WinHTTP proxy to direct mode..."
    Invoke-External -Exe "netsh" -ArgList @("winhttp","reset","proxy") -AllowFail | Out-Null

    if (Test-IsAdmin) {
        Info "Admin mode detected. Setting DNS on active physical adapters to Cloudflare/Google."
        try {
            ...
            foreach ($adapter in $adapters) {
                try {
                    ...
                    Ok "DNS set for adapter: $($adapter.Name)"
                } catch {
                    Warn "Could not set DNS for adapter $($adapter.Name): $($_.Exception.Message)"
                }
            }
        } catch {
            Warn "DNS adapter update failed: $($_.Exception.Message)"
        }
    } else {
        Warn "PowerShell is not running as Administrator, so adapter DNS cannot be changed automatically."
        Warn "If the next check fails, run this script from Administrator PowerShell or set adapter DNS manually to 1.1.1.1 and 8.8.8.8."
    }

    Info "Restarting Docker Desktop backend services if available..."
    ...
    Info "Waiting for Docker Engine to answer..."
    $ready = $false
    for ($i = 1; $i -le 60; $i++) {
        & docker info *> $null
    ...
    if ($ready) { Ok "Docker Engine is available." } else { Warn "Docker Engine did not become ready automatically." }
}

function Test-DockerHubAccess {
    Write-Host ""
    Write-Host "=== Docker Hub access preflight ==="

    $dnsOk = $false
    try {
        $dns = Resolve-DnsName "registry-1.docker.io" -Type A -ErrorAction Stop
        if ($dns) { $dnsOk = $true }
    } catch {
        Warn "Windows DNS cannot resolve registry-1.docker.io: $($_.Exception.Message)"
    }

    if ($dnsOk) {
        Ok "Windows DNS resolves registry-1.docker.io."
    } else {
        Repair-DockerDesktopNetwork
    }

    Info "Testing Docker pull of python:3.11-slim..."
    Invoke-External -Exe "docker" -ArgList @("pull","python:3.11-slim") -AllowFail | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Host ""
        Warn "Docker still cannot pull python:3.11-slim. This is a Docker Desktop/DNS/proxy issue, not a MiroFish code issue."
        Write-Host "Manual fix checklist:"
        Write-Host "1) Open PowerShell as Administrator."
        Write-Host "2) Run: ipconfig /flushdns"
        Write-Host "3) Run: netsh winhttp reset proxy"
        Write-Host "4) Set active network adapter DNS to 1.1.1.1 and 8.8.8.8."
        Write-Host "5) Docker Desktop -> Settings -> Resources -> Network -> DNS server: Manual, 1.1.1.1"
        Write-Host "6) Docker Desktop -> Troubleshoot -> Restart."
        Write-Host "7) Test: docker pull python:3.11-slim"
        throw "Docker Hub preflight failed: cannot pull python:3.11-slim"
    }
    Ok "Docker can pull python:3.11-slim."
}
...
Info "It checks and repairs Docker Desktop DNS/Docker Hub access before redeploy."
